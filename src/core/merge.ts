import {BaseLens, Lens, grabValue, Transformer} from "../internal"
...
> extends BaseLens<T> {
    constructor(public bases: BaseLens<any>[]) {
        super()
    }
...
export function merge<A, B>(lens: Lens<B>): Transformer<A, Lens<[A, B]>>
export function merge<A, B, C>(
    lens1: Lens<B>,
    lens2: Lens<C>
): Transformer<A, Lens<[A, B, C]>>
export function merge<A, B, C, D>(
    lens1: Lens<B>,
    lens2: Lens<C>,
    lens3: Lens<D>
): Transformer<A, Lens<[A, B, C, D]>>
export function merge<A, B, C, D, E>(
    lens1: Lens<B>,
    lens2: Lens<C>,
    lens3: Lens<D>,
    lens4: Lens<E>
): Transformer<A, Lens<[A, B, C, D, E]>>
export function merge<A, B, C, D, E, F>(
    ...
    lens4: Lens<E>,
    lens5: Lens<F>
): Transformer<A, Lens<[A, B, C, D, E, F]>>
export function merge(...lenses: Lens[]): Transformer<any, Lens<any[]>>
export function merge(...lenses: Lens[]): Transformer<any, any> {
    return function(lens: Lens<any>) {
        const allLenses = [lens].concat(lenses)
        const existing = mergeCache.get(getMergeCacheKey(allLenses))
        if (existing) return existing
        return new Merge(allLenses as any)
    }
}
